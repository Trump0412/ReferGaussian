...
def _ranges_total_len(segments: list[list[int]]) -> int:
...
def _dedupe_subject_selection(
    ...
    for entity_id, phrase in zip(subject_ids, subject_phrases):
        ...
        dedup_ids.append(entity_value)
        dedup_phrases.append(str(phrase))
...
def _select_phrase_ids(
    ...
    for target in normalized_targets:
        ...
        if not matches and ranked and _candidate_phrase_score(target, ranked[0]) >= 0.30:
            matches = [ranked[0]]
        if not matches and allow_missing and ranked and os.environ.get("QUERY_MISSING_PHRASE_PICK_BEST", "0") == "1":
            matches = sorted(
                ranked,
                key=lambda item: (
                    -float(item.get("quality", 0.0)),
                    -_ranges_total_len(item.get("support_segments_test", [])),
                    -_ranges_total_len(item.get("query_relevant_segments_test", [])),
                    -_ranges_total_len(item.get("moving_segments_test", [])),
                    int(item.get("id", -1)),
                ),
...
def _frame_margin_to_test_margin(
    query_plan_payload: dict[str, Any],
    test_times: np.ndarray,
    frame_margin: int,
) -> int:
    frame_margin = int(max(0, frame_margin))
    if frame_margin <= 0:
        return 0
    dataset_dir_value = query_plan_payload.get("dataset_dir")
    if not dataset_dir_value:
        return frame_margin
    try:
        all_ids, _all_times = _dataset_time_values(Path(dataset_dir_value))
    except Exception:
        return frame_margin
    stride = max(1.0, float(len(all_ids)) / max(float(len(test_times)), 1.0))
...
def _structural_before_uses_full_rendered_range(query_norm: str, boundary_end: int | None, frame_count: int) -> bool:
    # Historical note:
    # We briefly expanded some structural "before" queries (e.g. pre-cut lemon)
    ...
    compact = str(query_norm).replace(" ", "")
    fill_state_terms = (
        "加入任何液体之前",
        "倒入之前",
        "liquid",
    ...
    if any(term in query_norm for term in fill_state_terms) or any(term in compact for term in fill_state_terms):
        return False
    structural_terms = (
        "被切成两半之前",
        "切成两半之前",
    ...
    if not (any(term in query_norm for term in structural_terms) or any(term in compact for term in structural_terms)):
        return False
...
def _candidate_matches_phrase(candidate: dict[str, Any], phrase: str, threshold: float = 0.45) -> bool:
...
def _select_static_candidates(
    ...
    for candidate in candidates:
        support_len = _ranges_total_len(candidate.get("support_segments_test", []))
        moving_len = _ranges_total_len(candidate.get("moving_segments_test", []))
        stationary_len = _ranges_total_len(candidate.get("stationary_segments_test", []))
        ...
        if stationary_ratio < 0.70 and not (support_ratio >= 0.85 and moving_ratio <= 0.04):
            continue
        score = stationary_ratio + 0.25 * support_ratio - 0.45 * moving_ratio + 0.10 * float(candidate.get("quality", 0.0))
...
def _query_state_mode(query_norm: str) -> str | None:
    ...
    padded = f" {query_norm} "
    if (
        "above the midpoint" in query_norm
        or "above midpoint" in query_norm
        or "midpoint of the cup" in query_norm
        or "超过中点" in compact
        or "高于中点" in compact
        or "中点以上" in compact
        or "半杯以上" in compact
    ...
    if "full" in query_norm or any(token in compact for token in ("装满", "满杯", "完全满", "完全装满")):
        return "full"
    if "empty" in query_norm or any(token in compact for token in ("空杯", "空的", "空玻璃杯", "没有液体")):
        return "empty"
    if "light colored" in query_norm or "light-colored" in query_norm or "lighter" in query_norm:
        return "light"
    if "darker" in query_norm or " dark " in padded or query_norm.endswith(" dark") or any(token in compact for token in ("更深", "深色", "变深")):
        return "dark"
    ...
    if any(pattern in query_norm for pattern in before_patterns) or any(pattern in compact for pattern in before_patterns):
        return "before"
    after_patterns = (
        "after",
        "post ",
    ...
    if any(pattern in query_norm for pattern in after_patterns) or any(pattern in compact for pattern in after_patterns):
        return "after"
    action_patterns = (
        "during",
        "while",
    ...
    if any(pattern in query_norm for pattern in action_patterns) or any(pattern in compact for pattern in action_patterns):
        return "action"
...
def _track_state_segments_test(
    ...
    def _first_progress_onset(
        ...
        *,
        ...
        usable_mask = valid_mask if search_mask is None else np.asarray(search_mask, dtype=bool)
        active = usable_mask & (np.arange(frame_count) >= int(max(min_index, 0))) & (progress_array >= float(threshold))
    ...
    def _first_rise_onset(
        ...
        *,
        ...
        if frame_count <= 0:
            return None
        smoothed = _smooth_series(np.asarray(score_array, dtype=np.float32), radius=1)
        delta = np.diff(smoothed, prepend=smoothed[0]).astype(np.float32)
        active = (
            valid_mask
            & (np.arange(frame_count) >= int(max(min_index, 0)))
            & (delta >= float(delta_threshold))
            & (progress_array >= float(progress_floor))
        )
        indices = np.where(active)[0]
        if indices.size == 0:
            return None
        ...
        for onset in indices.tolist():
            ...
            if int(active_int[int(onset) : int(end)].sum()) >= int(max(sustain_count, 1)):
                metadata["delta_threshold"] = float(delta_threshold)
                metadata["progress_floor"] = float(progress_floor)
                return int(onset)
        return None
    ...
    if state_mode == "full":
        ...
        plan_min_index = int(plan_start or 0) if _env_flag("QUERY_STATE_USE_PLAN_MIN_INDEX", False) else 0
        min_index = max(int(frame_count * 0.25), plan_min_index)
        interpolated_search_mask = np.ones((frame_count,), dtype=bool)
        threshold = _env_float("QUERY_STATE_FULL_PROGRESS_THRESHOLD", 0.55, minimum=0.0, maximum=1.0)
        stabilize_frames = _env_int("QUERY_STATE_FULL_STABILIZE_FRAMES", 6, minimum=0, maximum=max(frame_count - 1, 0))
        onset = _first_progress_onset(
            ...
            sustain_window=3,
            sustain_count=2,
            search_mask=interpolated_search_mask,
        )
        if onset is None:
            onset = int(default_start)
        else:
            onset = min(max(frame_count - 1, 0), int(onset) + int(stabilize_frames))
        derived_mask = np.zeros((frame_count,), dtype=bool)
        derived_mask[max(0, int(onset)) :] = True
        metadata["threshold"] = float(threshold)
        metadata["stabilize_frames"] = int(stabilize_frames)
        metadata["progress_low"] = progress_low
        metadata["progress_high"] = progress_high
        metadata["strategy"] = "first_stable_fill_progress_interpolated"
        ...
        min_fraction = _env_float("QUERY_STATE_ABOVE_MIDPOINT_MIN_FRACTION", 0.50, minimum=0.0, maximum=1.0)
        threshold = _env_float("QUERY_STATE_ABOVE_MIDPOINT_PROGRESS_THRESHOLD", 0.14, minimum=0.0, maximum=1.0)
        sustain_window = _env_int("QUERY_STATE_ABOVE_MIDPOINT_SUSTAIN_WINDOW", 1, minimum=1, maximum=max(frame_count, 1))
        sustain_count = _env_int("QUERY_STATE_ABOVE_MIDPOINT_SUSTAIN_COUNT", 1, minimum=1, maximum=max(frame_count, 1))
        plan_min_index = int(plan_start or 0) if _env_flag("QUERY_STATE_USE_PLAN_MIN_INDEX", False) else 0
        min_index = max(int(frame_count * float(min_fraction)), plan_min_index)
        interpolated_search_mask = np.ones((frame_count,), dtype=bool)
        onset = _first_progress_onset(
            progress,
            threshold=threshold,
        ...
        derived_mask[max(0, int(onset)) :] = True
        metadata["threshold"] = float(threshold)
        metadata["min_fraction"] = float(min_fraction)
        metadata["sustain_window"] = int(sustain_window)
        metadata["sustain_count"] = int(sustain_count)
        metadata["plan_min_index_used"] = int(plan_min_index)
        metadata["progress_low"] = progress_low
        metadata["progress_high"] = progress_high
        metadata["strategy"] = "first_stable_midpoint_progress_interpolated"
        ...
        threshold = _env_float("QUERY_STATE_EMPTY_PROGRESS_THRESHOLD", 0.15, minimum=0.0, maximum=1.0)
        pre_onset_margin = _env_int("QUERY_STATE_EMPTY_PRE_ONSET_MARGIN", 8, minimum=0, maximum=max(frame_count - 1, 0))
        onset = _first_progress_onset(
            progress,
            threshold=threshold,
            min_index=min_index,
        )
        rise_threshold = max(float(np.quantile(np.diff(_smooth_series(score, radius=1), prepend=score[0])[valid_mask], 0.92)), 0.008)
        rise_onset = _first_rise_onset(
            score,
            progress,
            min_index=min_index,
            delta_threshold=rise_threshold,
            progress_floor=0.08,
        )
        if rise_onset is not None and (onset is None or int(rise_onset) + 2 < int(onset)):
            onset = int(rise_onset)
            metadata["strategy"] = "prefix_until_fill_rise"
        if onset is None:
            ...
            prefix_end = max(0, int(onset) - 1 - int(pre_onset_margin))
            metadata["strategy"] = "prefix_until_fill_onset"
        metadata["threshold"] = float(threshold)
        metadata["pre_onset_margin"] = int(pre_onset_margin)
        metadata["progress_low"] = progress_low
        metadata["progress_high"] = progress_high
        derived_mask = np.zeros((frame_count,), dtype=bool)
        ...
        if rise_onset is not None and (onset is None or int(rise_onset) + 2 < int(onset)):
            onset = int(rise_onset)
            metadata["strategy"] = "prefix_until_dark_rise"
        if onset is None:
            if not visible_segments:
                return [], {**metadata, "fallback": "support"}
            prefix_end = int(visible_segments[0][1])
        ...
        default_quantile = _env_float("QUERY_STATE_OPEN_SCORE_QUANTILE", 0.50, minimum=0.0, maximum=1.0)
        if state_mode == "opened":
            quantile = _env_float("QUERY_STATE_OPENED_SCORE_QUANTILE", default_quantile, minimum=0.0, maximum=1.0)
        else:
            quantile = _env_float("QUERY_STATE_CLOSED_SCORE_QUANTILE", default_quantile, minimum=0.0, maximum=1.0)
        threshold = float(np.quantile(score[valid_mask], quantile))
        ...
        metadata["quantile"] = float(quantile)
    else:
...
def _compose_phrase_grounded_selection(
    *,
    ...
    split_keywords = ("broken", "pieces", "halves", "split", "cracked", "掰断", "切开", "切成两半", "碎片", "断开")
    intact_keywords = ("complete", "whole", "intact", "unbroken", "完整", "未掰断", "未切开", "之前", "前的")
    if len(subject_ids) == 1:
        ...
        split_boundary_margin = _frame_margin_to_test_margin(
            query_plan_payload,
            test_times,
            _env_int(
                "QUERY_STATE_SPLIT_BOUNDARY_MARGIN_FRAMES",
                0,
                minimum=0,
                maximum=max(int(len(test_times) * 8), 0),
            ),
        )
        ...
        def _select_rows_for_target_segments(
            ...
            reason: str,
            *,
            allow_target_outside_available: bool = False,
        ) -> list[dict[str, Any]]:
            chosen_rows = []
            for row in matched_rows:
                ...
                if allow_target_outside_available and target_segments:
                    row_segments = _merge_ranges(target_segments)
                else:
                    row_segments = (
        ...
        if query_state_mode == "after" or (query_state_mode != "before" and any(keyword in query_norm for keyword in split_keywords)):
            ...
            selected_rows = _select_rows_for_target_segments(
                track_state_segments,
                f"Selected from track-derived {state_mode} support for subject phrase '{subject_phrases[0]}'.",
                allow_target_outside_available=state_mode in {"full", "above_midpoint", "dark", "light", "empty", "opened", "closed"},
            )
            ...
            pre_end = min(len(test_times) - 1, max(0, int(split_frame) - 1 + int(split_boundary_margin)))
            post_start = max(0, int(split_frame) - int(split_boundary_margin))
            ...
            if target_segments:
                ...
                selected_rows = _select_rows_for_target_segments(
                    target_segments,
                    f"Selected from track-derived {state_mode} support for subject phrase '{subject_phrases[0]}'.",
                    allow_target_outside_available=state_mode in {"full", "above_midpoint", "dark", "light", "empty"},
                )
                selection_source = f"single_subject_track_{state_mode}"
            else:
                selected_rows = _select_rows_for_target_segments(
