...
run_with_gpu_monitor "${LOG_PATH}" "${META_PATH}" \
  bash -lc "cd '${GS_ROOT}' && export PYTHONPATH='${PYTHONPATH}' && ${PY_CMD} external/4DGaussians/render.py -m '${RUN_DIR}' --iteration -1 --warp_enabled --temporal_warp_type 'refergaussian' --temporal_extent_enabled --temporal_gate_sharpness '${TEMPORAL_GATE_SHARPNESS:-1.0}' --temporal_drift_scale '${TEMPORAL_DRIFT_SCALE:-1.0}' --temporal_gate_mix '${TEMPORAL_GATE_MIX_VALUE}' --temporal_drift_mix '${TEMPORAL_DRIFT_MIX_VALUE}' --temporal_tube_enabled --temporal_tube_samples '${TEMPORAL_TUBE_SAMPLES:-3}' --temporal_tube_span '${TEMPORAL_TUBE_SPAN:-0.40}' --temporal_tube_sigma '${TEMPORAL_TUBE_SIGMA:-0.34}' --temporal_tube_weight_power '${TEMPORAL_TUBE_WEIGHT_POWER:-1.0}' --temporal_tube_covariance_mix '${TEMPORAL_TUBE_COVARIANCE_MIX:-0.05}' --skip_train --skip_video --only_split test --stream_write --no_video_file ${ACCEL_ARGS} ${EXTRA_ARGS}"
