...
TEMPORAL_WARP_LR_INIT_VALUE="${TEMPORAL_WARP_LR_INIT:-0.00016}"
TEMPORAL_WARP_LR_SCHEDULE_VALUE="${TEMPORAL_WARP_LR_SCHEDULE:-constant}"
...
temporal_warp_lr_init: ${TEMPORAL_WARP_LR_INIT_VALUE}
temporal_warp_lr_schedule: ${TEMPORAL_WARP_LR_SCHEDULE_VALUE}
temporal_lr_init: ${TEMPORAL_LR_INIT:-0.00012}
temporal_lr_final: ${TEMPORAL_LR_FINAL:-0.000012}
...
temporal_tube_samples: ${TEMPORAL_TUBE_SAMPLES:-3}
temporal_tube_span: ${TEMPORAL_TUBE_SPAN:-0.40}
temporal_tube_sigma: ${TEMPORAL_TUBE_SIGMA:-0.34}
temporal_tube_weight_power: ${TEMPORAL_TUBE_WEIGHT_POWER:-1.0}
temporal_tube_covariance_mix: ${TEMPORAL_TUBE_COVARIANCE_MIX:-0.05}
EOF
...
run_with_gpu_monitor "${LOG_PATH}" "${META_PATH}" \
  bash -lc "cd '${GS_ROOT}' && export PYTHONPATH='${PYTHONPATH}' && ${PY_CMD} external/4DGaussians/train.py -s '${SOURCE_PATH}' -m '${RUN_DIR}' --expname 'refergaussian/${DATASET}/${SCENE##*/}' --configs '${CONFIG_PATH}' --port '${PORT_VALUE}' --seed '${TRAIN_SEED}' --iterations '${TRAIN_ITERATIONS}' --warp_enabled --temporal_warp_type 'refergaussian' --warp_hidden_dim '${WARP_HIDDEN_DIM:-32}' --warp_num_layers '${WARP_NUM_LAYERS:-2}' --warp_num_bins '${WARP_NUM_BINS:-128}' --warp_mono_weight '${WARP_MONO_WEIGHT:-0.05}' --warp_smooth_weight '${WARP_SMOOTH_WEIGHT:-0.01}' --warp_budget_weight '${WARP_BUDGET_WEIGHT:-0.01}' --warp_sample_count '${WARP_SAMPLE_COUNT:-128}' --temporal_warp_lr_init '${TEMPORAL_WARP_LR_INIT_VALUE}' --temporal_warp_lr_schedule '${TEMPORAL_WARP_LR_SCHEDULE_VALUE}' --temporal_lr_init '${TEMPORAL_LR_INIT:-0.00012}' --temporal_lr_final '${TEMPORAL_LR_FINAL:-0.000012}' --temporal_lr_delay_mult '${TEMPORAL_LR_DELAY_MULT:-0.01}' --temporal_extent_enabled --temporal_gate_sharpness '${TEMPORAL_GATE_SHARPNESS:-1.0}' --temporal_drift_scale '${TEMPORAL_DRIFT_SCALE:-1.0}' --temporal_gate_mix '${TEMPORAL_GATE_MIX_VALUE}' --temporal_drift_mix '${TEMPORAL_DRIFT_MIX_VALUE}' --temporal_velocity_reg_weight '${TEMPORAL_VELOCITY_REG_WEIGHT:-0.0}' --temporal_tube_enabled --temporal_tube_samples '${TEMPORAL_TUBE_SAMPLES:-3}' --temporal_tube_span '${TEMPORAL_TUBE_SPAN:-0.40}' --temporal_tube_sigma '${TEMPORAL_TUBE_SIGMA:-0.34}' --temporal_tube_weight_power '${TEMPORAL_TUBE_WEIGHT_POWER:-1.0}' --temporal_tube_covariance_mix '${TEMPORAL_TUBE_COVARIANCE_MIX:-0.05}' ${ACCEL_ARGS} ${EXTRA_ARGS}"
