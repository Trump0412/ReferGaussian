...
def run_grounded_sam2_query(
    ...
    for phrase_index, phrase in enumerate(detector_phrases):
        ...
        for anchor_index, anchor_detection in enumerate(selected_anchors):
            ...
            def _accumulate_video_segments(*, reverse: bool) -> bool:
                kwargs = {"reverse": True} if reverse else {}
                try:
                    segment_iterator = video_predictor.propagate_in_video(inference_state, **kwargs)
                except TypeError:
                    if reverse:
                        return False
                    raise
                for out_frame_idx, out_obj_ids, out_mask_logits in segment_iterator:
            ...
            _accumulate_video_segments(reverse=False)
            reverse_propagated = False
            if local_anchor_index > 0:
                reverse_propagated = _accumulate_video_segments(reverse=True)
            ...
            anchor_rows.append(
                {
                    ...
                    "reverse_propagated": bool(reverse_propagated),
                    "track_window": {
